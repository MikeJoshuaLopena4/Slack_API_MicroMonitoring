...
const toCSV = function (table) {
    ...
    const table_data = [...tbody_rows].map(row => {
        ...
        const data_without_img = [...cells].map(cell => {
            let cellContent = cell.textContent.trim();
            // Check if the cell content is a valid date
            if (/^\d{4}-\d{2}-\d{2}$/.test(cellContent)) {
                // If it's a date, enclose it in quotation marks
                return `"${cellContent}"`;
            } else {
...
sr.reveal('.info-img', { delay: 450, origin: 'top' });
